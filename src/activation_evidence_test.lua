...
local function define_tests()
    test.describe("Dataflow admission evidence", function()
        ...
        test.it("ensures the same admission without advancing generation and rejects another key", function()
            ...
            test.is_nil(other)
            test.is_true(errors.is(conflict, errors.CONFLICT))
            local wrong_read, read_conflict = activation_repo.get_activation_evidence(
                id, key .. ":other")
            test.is_nil(wrong_read)
            test.is_true(errors.is(read_conflict, errors.CONFLICT))
        ...
        test.it("retains terminal status and outcome until matching acknowledgment", function()
            ...
            local premature_ack, premature_err = activation_repo.ack_terminal(
                id, key, activation.generation, now())
            test.is_nil(premature_ack)
            test.is_true(errors.is(premature_err, errors.UNAVAILABLE))
        ...
        test.it("fences DELETE_WORKFLOW until terminal evidence is acknowledged", function()
            local id = create_flow()
            local key = "run:" .. uuid.v7()
            local activation, activation_err = activation_repo.ensure_activation(id, key, now())
            test.is_nil(activation_err)
            local terminal, terminal_err = with_tx(function(tx)
                ...
                if db_type == "postgres" then
                    query = "UPDATE dataflows SET status = $1, metadata = $2 WHERE dataflow_id = $3"
                end
                local _, update_err = tx:execute(query,
                    { consts.STATUS.COMPLETED_SUCCESS, '{"value":42}', id })
            ...
            local command = { type = consts.COMMAND_TYPES.DELETE_WORKFLOW, payload = {} }
            local deleted, delete_err = with_tx(function(tx)
                return ops.execute(tx, id, nil, command)
            end)
            test.is_nil(deleted)
            test.is_true(errors.is(delete_err, errors.CONFLICT))
            local evidence, evidence_err = activation_repo.get_activation_evidence(id, key)
            test.is_nil(evidence_err)
            test.eq(evidence.state, "terminal")
            test.eq(evidence.terminal_outcome.value, 42)
            ...
            test.is_nil(ack_err)
            test.is_true(ack.acknowledged)
            deleted, delete_err = with_tx(function(tx)
                return ops.execute(tx, id, nil, command)
            end)
            test.is_nil(delete_err)
            test.is_true(deleted.results[1].deleted)
            local absent, absent_err = activation_repo.get_activation_evidence(id, key)
            test.is_nil(absent_err)
