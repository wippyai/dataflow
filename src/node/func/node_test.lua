...
local function define_tests()
    describe("Function Node Integration Tests", function()
        ...
        describe("Control Command Tests", function()
            it("should execute control function and return child output", function()
                print("=== CONTROL FUNCTION TEST START ===")
                ...
                local workflow_commands = {
                    {
                        ...
                        payload = {
                            ...
                            status = consts.STATUS.PENDING,
                            config = {
                                func_id = "userspace.dataflow.node.func:control_test_func",
                                data_targets = {
                                    {
                                        data_type = consts.DATA_TYPE.WORKFLOW_OUTPUT,
                                        key = "result",
                            ...
                            metadata = {
                                title = "Control Function Test Node"
                            }
                        }
                    },
                    {
                ...
                local dataflow_id, create_err = c:create_workflow(workflow_commands, {
                    metadata = {
                        title = "Control Function Test Workflow"
                    }
                })
                test.is_nil(create_err)
                test.not_nil(dataflow_id)
                ...
                -- Verify output was created and contains child node result
                local output_data = data_reader.with_dataflow(dataflow_id)
                    :with_data_types(consts.DATA_TYPE.WORKFLOW_OUTPUT)
                    :fetch_options({ replace_references = true })
                    :one()
                ...
                -- Should contain the result from the child node spawned by control commands
                test.eq(output_content.message, "Control test")
                test.eq(output_content.processed_by, "test_function")
                test.is_true(output_content.success)

                print("Control function executed and returned child output")
            end)

            it("should execute chain workflow via control commands", function()
                local c, err = client.new()
                test.is_nil(err)
                ...
                local dataflow_id, create_err = c:create_workflow(workflow_commands, {
                    metadata = {
                        title = "Chain Control Test Workflow"
                    }
                })
                test.is_nil(create_err)
                ...
                -- Verify the chain workflow executed and final output contains processed data
                local output_data = data_reader.with_dataflow(dataflow_id)
                    :with_data_types(consts.DATA_TYPE.WORKFLOW_OUTPUT)
                    :fetch_options({ replace_references = true })
                    :one()
                ...
                -- Final output from the end of the chain
                test.eq(output_content.processed_by, "test_function")
                test.is_true(output_content.success)

                print("Chain control workflow executed successfully")
            end)

            it("should execute parallel workflow via control commands", function()
                local c, err = client.new()
                test.is_nil(err)
                ...
                local workflow_commands = {
                    {
                        ...
                        payload = {
                            ...
                            metadata = {
                                title = "Parallel Control Test Node"
                            }
                        }
                    },
                    {
                ...
                local dataflow_id, create_err = c:create_workflow(workflow_commands, {
                    metadata = {
                        title = "Parallel Control Test Workflow"
                    }
                })
                test.is_nil(create_err)
                ...
                -- Verify parallel workflow executed and we got outputs from both branches
                local output_data = data_reader.with_dataflow(dataflow_id)
                    :with_data_types(consts.DATA_TYPE.WORKFLOW_OUTPUT)
                    :fetch_options({ replace_references = true })
                    :one()
                ...
                -- Should be an array with results from both parallel branches
                test.eq(type(output_content), "table")
                test.eq(#output_content, 2)

                -- Both should be successful test function results
                for _, branch_result in ipairs(output_content) do
                    local resolved: any = branch_result
                    if type(resolved) == "string" then
                        local decoded, _decode_err = json.decode(resolved)
                        if not _decode_err then
                            resolved = decoded
                        end
                    end
                    test.eq(resolved.processed_by, "test_function")
                    test.is_true(resolved.success)
                end

                print("Parallel control workflow executed successfully")
            end)
        end)
    end)
end
