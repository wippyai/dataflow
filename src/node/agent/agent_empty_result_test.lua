...
local function define_tests()
    describe("Agent Node empty final turn", function()
        ...
        local function create_workflow(opts)
            ...
            local node_config: any = {
                ...
                arena = {
                    ...
                    max_unproductive_steps = opts.max_unproductive_steps,
                    tool_calling = opts.tool_calling or "auto",
                    tools = {
        ...
        local function agent_error(workflow)
            ...
            return as_table(as_table((agent_result :: any).content).error)
        end
        ...
        it("fails on the consecutive unproductive bound instead of spinning to the iteration cap", function()
            local workflow = create_workflow({
            ...
            test.eq((agent_nodes[1] :: any).status, consts.STATUS.COMPLETED_FAILURE,
                "agent node fails on the unproductive-turn bound")

            local feedback = empty_result_observations(workflow)
            test.eq(#feedback, 3, "every empty turn is answered with empty-result feedback")

            local result_error = agent_error(workflow)
            test.eq(result_error.code, agent_consts.ERROR.UNPRODUCTIVE_STEPS, "the failure carries the named code")
            test.contains(tostring(result_error.message), "3 consecutive turns",
                "the failure names how many unproductive turns were seen")

            test.eq(#outputs_for(workflow, "result"), 0, "no result output is produced from empty turns")
        end)

        it("routes the unproductive-turn failure through the node error target", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 12,
                max_unproductive_steps = 3,
            ...
            test.eq(routed_error.code, agent_consts.ERROR.UNPRODUCTIVE_STEPS,
        ...
        it("honors a configured bound", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 12,
                max_unproductive_steps = 2
            })

            c:start(workflow.dataflow_id)

            test.eq(wait_terminal(workflow.dataflow_id), consts.STATUS.COMPLETED_FAILURE,
                "empty turns never complete the workflow")
            test.eq(get_metrics(workflow.scenario_id).llm_calls, 2, "the model is called exactly the configured bound")
            test.eq(#empty_result_observations(workflow), 2, "every counted turn is answered with feedback")

            local result_error = agent_error(workflow)
            test.eq(result_error.code, agent_consts.ERROR.UNPRODUCTIVE_STEPS, "the failure carries the named code")
            test.contains(tostring(result_error.message), "2 consecutive turns",
                "the failure names how many unproductive turns were seen")
        end)

        it("bounds unproductive turns under tool_calling none", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 12,
                max_unproductive_steps = 3,
                tool_calling = agent_consts.TOOL_CALLING.NONE
            })
            ...
            test.eq(wait_terminal(workflow.dataflow_id), consts.STATUS.COMPLETED_FAILURE,
                "empty turns never complete the workflow")
            test.eq(get_metrics(workflow.scenario_id).llm_calls, 3, "the model is called exactly the bounded number of times")
            test.eq(#empty_result_observations(workflow), 0, "none mode rejects empty turns without feedback")
            test.eq(agent_error(workflow).code, agent_consts.ERROR.UNPRODUCTIVE_STEPS,
                "the failure carries the named code")
        end)

        it("keeps the iteration cap when it is tighter than the bound", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 2,
            ...
            test.eq(wait_terminal(workflow.dataflow_id), consts.STATUS.COMPLETED_FAILURE,
                "empty turns never complete the workflow")
            test.eq(get_metrics(workflow.scenario_id).llm_calls, 2, "the run stops at max_iterations")
            test.eq(agent_error(workflow).code, agent_consts.ERROR.AGENT_EXEC_FAILED,
                "the iteration cap reports its own failure")
        end)

        it("continues the persisted unproductive run when the node resumes", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 12,
                max_unproductive_steps = 3,
                state = { current_iteration = 2, unproductive_steps = 2 }
            })
            ...
            test.eq(wait_terminal(workflow.dataflow_id), consts.STATUS.COMPLETED_FAILURE,
                "empty turns never complete the workflow")
            test.eq(get_metrics(workflow.scenario_id).llm_calls, 1,
                "one more unproductive turn exhausts the allowance carried in node state")

            local result_error = agent_error(workflow)
            test.eq(result_error.code, agent_consts.ERROR.UNPRODUCTIVE_STEPS, "the failure carries the named code")
            test.contains(tostring(result_error.message), "3 consecutive turns", "the count spans the resume")
        end)

        it("fails a resumed node whose persisted run already exhausted the allowance", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 12,
                max_unproductive_steps = 3,
                state = { current_iteration = 3, unproductive_steps = 3 }
            })

            c:start(workflow.dataflow_id)

            test.eq(wait_terminal(workflow.dataflow_id), consts.STATUS.COMPLETED_FAILURE,
                "the resumed node fails")
            test.eq(get_metrics(workflow.scenario_id).llm_calls or 0, 0, "the model is not asked again")
            test.eq(agent_error(workflow).code, agent_consts.ERROR.UNPRODUCTIVE_STEPS,
                "the failure carries the named code")
        end)

        it("rejects a bound that is not a positive integer", function()
            local workflow = create_workflow({
                mode = "empty_until_limit",
                max_iterations = 12,
                max_unproductive_steps = 0
            })

            c:start(workflow.dataflow_id)

            test.eq(wait_terminal(workflow.dataflow_id), consts.STATUS.COMPLETED_FAILURE,
                "an invalid bound fails the node")
            test.eq(get_metrics(workflow.scenario_id).llm_calls or 0, 0, "the model is never called")

            local result_error = agent_error(workflow)
            test.eq(result_error.code, agent_consts.ERROR.INVALID_CONFIG, "the failure is a configuration error")
            test.contains(tostring(result_error.message), "max_unproductive_steps", "the failure names the setting")
        end)
    end)
end
